...
Dada la aparente simetría en los histogramas, simplificar las mediciones de dichas variables con sus respectivos promedios y medianas no es una idea 
descabellada.
...
primeros tres meses del año. Una vez agrupadas las ubicaciones, se promedia y
se toma la mediana de todos los valores medidos en dichas ubicaciones de las 
variables de interés. La base de datos resultante tiene la siguiente estructura
...
mientras que el esférico y exponencial presentan comportamientos similares.
...
```{r}
datos_geo_copy <- as.geodata(datos_def, coords.col = 7:8,
                       data.col = 5)
plot(datos_geo_copy)
```
...
comparison %>% 
  kable(col.names = c("Modelo", "RMSE"),
        caption = "Comparación de modelos", row.names = F) %>%
  kable_styling(bootstrap_options = c("striped", "hover"))
```
