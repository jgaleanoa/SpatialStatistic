...
```{r}
# Listas vacias
map_image_gaus.uv <- list()
map_image_exps.uv <- list()
map_image_spes.uv <- list()

krig_gaus.uv <- list()
krig_exps.uv <- list()
krig_spes.uv <- list()

map_image_gausvar.uv <- list()
map_image_expsvar.uv <- list()
map_image_spesvar.uv <- list()

for(i in 1:4){
  # Modelo gaussiano
  krig_gaus.uv[[i]] <- krige.conv(datos_geo, data = datos_geo$data[, i],
                               loc = prediction_grid,
                          krige = 
                            krige.control(obj.model = model_mco_gaus[[i]],
                                          trend.d = trend.spatial("~ y", datos_geo))
                          )
  
  map_image_gaus.uv[[i]] <- rasterFromXYZ(data.frame(prediction_grid,
                                             krig_gaus.uv[[i]]$predict))
  
  map_image_gausvar.uv[[i]] <- rasterFromXYZ(data.frame(prediction_grid,
                                             sqrt(krig_gaus.uv[[i]]$krige.var)))
  #Modelo Exponencial
  krig_exps.uv[[i]] <- krige.conv(datos_geo, data = datos_geo$data[, i],
                               loc = prediction_grid,
                          krige = 
                            krige.control(obj.model = model_mco_exps[[i]]))
  
  map_image_exps.uv[[i]] <- rasterFromXYZ(data.frame(prediction_grid,
                                             krig_exps.uv[[i]]$predict))
  
  map_image_expsvar.uv[[i]] <- rasterFromXYZ(data.frame(prediction_grid,
                                             sqrt(krig_exps.uv[[i]]$krige.var)))
  
  # Modelo esferico
  krig_spes.uv[[i]] <- krige.conv(datos_geo, data = datos_geo$data[, i],
                               loc = prediction_grid,
                          krige = 
                            krige.control(obj.model = model_mco_spes[[i]]))
  
  map_image_spes.uv[[i]] <- rasterFromXYZ(data.frame(prediction_grid,
                                             krig_spes.uv[[i]]$predict))
  
   map_image_spesvar.uv[[i]] <- rasterFromXYZ(data.frame(prediction_grid,
                                             sqrt(krig_spes.uv[[i]]$krige.var)))
}
```
